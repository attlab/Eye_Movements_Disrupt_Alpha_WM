...
iemDir = [rDir '/' 'IEM_Fix_Locked' ];
eyeDir = [rDir '/' 'EYE/Eye_Fixation_Mats'];
% select subjects
%subjects = 4;
...
for s=1:length(subjects)
    ...
    % load eye-tracking fixation locking data
    for iCond=3
       allCondsEye(iCond) = load([eyeDir '/' sprintf('sj%02d_cond%02d.mat',sn,iCond)]); 
    end
    
    % add session data to both trial mats [HARD CODED - need to add session earlier in pipelines]
    for i=1:length(allCondsEye(3).trialData)
        if ismember(i,1:254)
            allCondsEye(3).trialData(i).thisSession=1;
        else
            allCondsEye(3).trialData(i).thisSession=2;
        end  
    end
    
    for i=1:length(allConds(3).beh)
        if ismember(i,1:234)
            allConds(3).beh(i).thisSession=1;
        else
            allConds(3).beh(i).thisSession=2;
        end
    end
    
    % create a unique trial code for each mat
    for i=1:length(allCondsEye(3).trialData)
        if      allCondsEye(3).trialData(i).thisSession==1 && allCondsEye(3).trialData(i).thisBlock==1; allCondsEye(3).trialData(i).cTrial=allCondsEye(3).trialData(i).thisTrial +1000;
        elseif  allCondsEye(3).trialData(i).thisSession==1 && allCondsEye(3).trialData(i).thisBlock==2; allCondsEye(3).trialData(i).cTrial=allCondsEye(3).trialData(i).thisTrial +2000;
        elseif  allCondsEye(3).trialData(i).thisSession==1 && allCondsEye(3).trialData(i).thisBlock==3; allCondsEye(3).trialData(i).cTrial=allCondsEye(3).trialData(i).thisTrial +3000;
        elseif  allCondsEye(3).trialData(i).thisSession==1 && allCondsEye(3).trialData(i).thisBlock==4; allCondsEye(3).trialData(i).cTrial=allCondsEye(3).trialData(i).thisTrial +4000;
        elseif  allCondsEye(3).trialData(i).thisSession==2 && allCondsEye(3).trialData(i).thisBlock==1; allCondsEye(3).trialData(i).cTrial=allCondsEye(3).trialData(i).thisTrial +5000;
        elseif  allCondsEye(3).trialData(i).thisSession==2 && allCondsEye(3).trialData(i).thisBlock==2; allCondsEye(3).trialData(i).cTrial=allCondsEye(3).trialData(i).thisTrial +6000;
        elseif  allCondsEye(3).trialData(i).thisSession==2 && allCondsEye(3).trialData(i).thisBlock==3; allCondsEye(3).trialData(i).cTrial=allCondsEye(3).trialData(i).thisTrial +7000;
        elseif  allCondsEye(3).trialData(i).thisSession==2 && allCondsEye(3).trialData(i).thisBlock==4; allCondsEye(3).trialData(i).cTrial=allCondsEye(3).trialData(i).thisTrial +8000;
        end
    end
 
    for i=1:length(allConds(3).beh)
        if      allConds(3).beh(i).thisSession==1 && allConds(3).beh(i).thisBlock==1; allConds(3).beh(i).cTrial=allConds(3).beh(i).thisTrial +1000;
        elseif  allConds(3).beh(i).thisSession==1 && allConds(3).beh(i).thisBlock==2; allConds(3).beh(i).cTrial=allConds(3).beh(i).thisTrial +2000;
        elseif  allConds(3).beh(i).thisSession==1 && allConds(3).beh(i).thisBlock==3; allConds(3).beh(i).cTrial=allConds(3).beh(i).thisTrial +3000;
        elseif  allConds(3).beh(i).thisSession==1 && allConds(3).beh(i).thisBlock==4; allConds(3).beh(i).cTrial=allConds(3).beh(i).thisTrial +4000;
        elseif  allConds(3).beh(i).thisSession==2 && allConds(3).beh(i).thisBlock==1; allConds(3).beh(i).cTrial=allConds(3).beh(i).thisTrial +5000;
        elseif  allConds(3).beh(i).thisSession==2 && allConds(3).beh(i).thisBlock==2; allConds(3).beh(i).cTrial=allConds(3).beh(i).thisTrial +6000;
        elseif  allConds(3).beh(i).thisSession==2 && allConds(3).beh(i).thisBlock==3; allConds(3).beh(i).cTrial=allConds(3).beh(i).thisTrial +7000;
        elseif  allConds(3).beh(i).thisSession==2 && allConds(3).beh(i).thisBlock==4; allConds(3).beh(i).cTrial=allConds(3).beh(i).thisTrial +8000;
        end
    end
...
%     for i=1:length(allCondsEye(3).trialData)
%         
%         if allCondsEye(3).trialData(i).cTrial==allConds(3).beh(i).cTrial
%         
%         
%         
%         end
...
%     end
    ...
    % find trials that are missing from each set and remove to balance them
    [a,thisDiffBehIdx] =  setdiff([allConds(3).beh.cTrial],[allCondsEye(3).trialData.cTrial]);
    [b,thisDiffTrialDataIdx] =  setdiff([allCondsEye(3).trialData.cTrial],[allConds(3).beh.cTrial]);

    allConds(3).beh(thisDiffBehIdx) = [];
    allCondsEye(3).trialData(thisDiffTrialDataIdx) = [];
    allCondsEye(3).fixStruct(thisDiffTrialDataIdx) = [];

    % remove missing trials from eegs also
    allConds(3).eegs(:,:,thisDiffBehIdx) = [];

    % now remove bad trials as ID'd in the fixation structure
    moreBadTrialsIdx = find([allCondsEye(3).fixStruct.badTrial]==1);
    
    allCondsEye(3).trialData(moreBadTrialsIdx) = [];
    allCondsEye(3).fixStruct(moreBadTrialsIdx) = [];
    allConds(3).eegs(:,:,moreBadTrialsIdx) = [];
    allConds(3).beh(moreBadTrialsIdx) = [];

   
   %% NOW ALL BAD TRIALS REMOVED!!!
...
%         minCnt = min(minCntMat)-1; % minus 1 to give all bins variability across iterations
        ...
        minCnt = allConds(3).minCnt;
        
        
        %% loop through eye movement chunks

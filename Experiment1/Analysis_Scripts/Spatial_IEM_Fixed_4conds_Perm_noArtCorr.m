function Spatial_IEM_Fixed_4conds_Perm_noArtCorr(sjNum,iSpatialOrColor)
...
eegDir = [root '/' 'EEG_Prepro2_Avg_Baseline_noArtCorr'];
...
%     % add exceptions for sjs 8 and 10 (noArtCorr specific)
%     if sjNum==10 && iSession==1
%         allBehStruct = allBehStruct(1:953);
%     elseif sjNum==17 && iSession==1
%         allBehStruct = allBehStruct(1:1023);
%     end
...
for iCond=1:4
    ...
    for i=1:length(beh)
        if iSpatialOrColor==1
            posBin(i)=beh(i).stimLoc; % get sim location codes
        else
            posBin(i)=beh(i).colorListBin; % get color bin codes
        end
    end
...
if iSpatialOrColor==1
    ...
    cond1_beh = [allConds(1).beh.colorListBin];
    cond2_beh = [allConds(2).beh.colorListBin];
    cond3_beh = [allConds(3).beh.colorListBin];
    cond4_beh = [allConds(4).beh.colorListBin];
end
...
%             % permute position bins
%             for iPerm=1:7
%                 tmp = randperm(length(posBin));
%                 posBin = posBin(tmp);
%             end
%             
            ...
            for ii = 1:nBins
                for iii = 1:nBlocks
                    ...
                    if (iii == 1 || iii == 2) % cond1 condition
                        ...
                        blockDat_total_cond1(cond1_bCnt,:,:) = abs(squeeze(mean(fdata_total(permedPosBin==posBins(ii) & blocks==iii & perm_Trials == 1,:,tois),1)));
                    ...
                    elseif (iii == 3 || iii == 4) % Exercise condition
                        ...
                        blockDat_total_cond2(cond2_bCnt,:,:) = abs(squeeze(mean(fdata_total(permedPosBin==posBins(ii) & blocks==iii& perm_Trials == 2,:,tois),1)));
                    ...
                    elseif (iii == 5 || iii == 6) % Exercise condition
                        ...
                        blockDat_total_cond3(cond3_bCnt,:,:) = abs(squeeze(mean(fdata_total(permedPosBin==posBins(ii) & blocks==iii& perm_Trials == 3,:,tois),1)));
                    ...
                    elseif (iii == 7 || iii == 8) % Exercise condition
                        ...
                        blockDat_total_cond4(cond4_bCnt,:,:) = abs(squeeze(mean(fdata_total(permedPosBin==posBins(ii) & blocks==iii& perm_Trials == 4,:,tois),1)));
            ...
            for t = 1:nSamps
                ...
                for i=1:nBlocks % loop through blocks, holding each out as the test set
                    ...
                    if i == 1 || i == 2
                        ...
                        tf_total_cond1(f,iter,perm,t,cond1Total_i,:) = mean(C2_cond1,1);
                    ...
                    elseif i == 3 || i == 4
                        ...
                        tf_total_cond2 (f,iter,perm,t,cond2Total_i,:)  = mean(C2_cond2,1);
                    ...
                    elseif i == 5 || i == 6
                        ...
                        tf_total_cond3 (f,iter,perm,t,cond3Total_i,:)  = mean(C2_cond3,1);
                    ...
                    elseif i == 7 || i == 8
                        ...
                        tf_total_cond4 (f,iter,perm,t,cond4Total_i,:)  = mean(C2_cond4,1);
...
tf_total_cond1 = squeeze(mean(mean(mean(tf_total_cond1,2),3),5));
tf_total_cond2 =  squeeze(mean(mean(mean(tf_total_cond2,2),3),5));
tf_total_cond3 = squeeze(mean(mean(mean(tf_total_cond3,2),3),5));
tf_total_cond4 =  squeeze(mean(mean(mean(tf_total_cond4,2),3),5));

cd([root '/Analysis_Scripts'])
% save data
if processForIEM==1
    
    %fName = [root, '/IEM_Results_TT_Within_Fixed_Perm/',sprintf('sj%02d_fixed_IEM.mat',sjNum)];
    
    if iSpatialOrColor==1
        fName = [root, '/IEM_Results_TT_Within_Fixed_Perm_noArtCorr/',sprintf('sj%02d_fixed_IEM.mat',sjNum)];
    else
        fName = [root, '/IEM_Results_TT_Within_Fixed_Perm_Color_noArtCorr/',sprintf('sj%02d_fixed_IEM.mat',sjNum)];
    end

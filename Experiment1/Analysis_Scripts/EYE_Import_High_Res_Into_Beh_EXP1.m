function EYE_Import_High_Res_Into_Beh_EXP1(sjNum)
%{
...
% set dirs
sourceDirBeh = '/home/waldrop/Desktop/WTF_EYE/Beh_Data_Processed';
sourceDirEye = '/home/waldrop/Desktop/WTF_EYE/EYE/Raw_EYE';
destDir = '/home/waldrop/Desktop/WTF_EYE/Beh_Data_Processed_Eye_Imported';
...
for iSession=1:2
    for iMemCond=1:2
        for iEyeCond=[2,6]
            ...
            % load eye data
            load([sourceDirEye '/' sprintf('s%d_%d%d%d.mat',sjNum,iSession,iMemCond,iEyeCond)])
            disp(['Loading Eye File ' sprintf('s%d_%d%d%d.mat',sjNum,iSession,iMemCond,iEyeCond)])  
            
            edfStruct = eyeMat.RawEdf;
            ...
            if edfStruct.FSAMPLE.time(2) - edfStruct.FSAMPLE.time(1) == 4
                disp('Upsample Data 250 Hz > 1000 Hz')
                ...
                upsampledDataPresent = 250;
            end
            
            if edfStruct.FSAMPLE.time(2) - edfStruct.FSAMPLE.time(1) == 2
                disp('Upsample Data 500 Hz > 1000 Hz')
                edfStruct.FSAMPLE.time = repelem(edfStruct.FSAMPLE.time,1,2);
                edfStruct.FSAMPLE.pa = repelem(edfStruct.FSAMPLE.pa,1,2);
                edfStruct.FSAMPLE.gx = repelem(edfStruct.FSAMPLE.gx,1,2);
                edfStruct.FSAMPLE.gy = repelem(edfStruct.FSAMPLE.gy,1,2);
                upsampledDataPresent = 500;
            end
            ...
            for iTrial=1:length(masterStruct(iSession,condCode).allTrialData)
                if masterStruct(iSession,condCode).allTrialData(iTrial).brokenTrial~=1
                    ...
                    %else
                        ...
                        if length(trialTimeIdx) <2500; trialTimeIdx(2500) = trialTimeIdx(2499); end
                    %end

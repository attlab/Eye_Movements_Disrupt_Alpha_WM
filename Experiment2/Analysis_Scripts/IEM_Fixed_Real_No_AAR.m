function IEM_Fixed_Real_No_AAR(sjNum,iSpatialOrColor)
...
for iSession=1:2
    ...
    eegs = eegs(:,:,~eeg.arf.artIndCleaned); % rejects trials
...
% find location/color bin with min count per condition (to balance IEM)
for iCond=1:4
...
if processForIEM==1
    ...
    if iSpatialOrColor==1
        fName = [root, '/IEM_Results_TT_Within_Fixed_No_AAR/',sprintf('sj%02d_fixed_IEM.mat',sjNum)];
    else
        fName = [root, '/IEM_Results_TT_Within_Fixed_Color/',sprintf('sj%02d_fixed_IEM.mat',sjNum)];
    end

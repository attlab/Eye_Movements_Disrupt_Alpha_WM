...
% generate plots
h=figure('OuterPosition',[679   542   894   381]);


for iPlot=1:2
    
   if       iPlot==1; theseData = modelSD; thisTitle = 'Precision'; thisYlim = [4,22]; thisYtick = [4,8,12,16,20]; 
   elseif   iPlot==2; theseData = modelGuess; thisTitle = 'Guess Rate'; thisYlim = [0,.22]; thisYtick = [0,.04,.08,.12,.16,.20]; 
   end
   subplot(1,2,iPlot)
   ...
   set(findall(1,'type','line','color','k'),'markerSize',12) %Change marker size
   ...
   xticklabelsNEW = {'S/F','C/F','S/M','C/M'};
   %xticklabelsNEW = {' '};%{'S/F','S/M','C/F','C/M'};
   
   set(gca,'xticklabels',xticklabelsNEW)

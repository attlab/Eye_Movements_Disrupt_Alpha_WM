...
function EEG_Preprocessing2_No_AAR(sjNum)
...
if whichPreprocessing==0 % average baseline (use for main spectral\\IEM analyses)
    destDir = [rDir '/' 'EEG_Prepro2_Avg_Baseline_No_AAR'];
...
% load artifact rejection mat (from main analysis)
load(['/home/waldrop/Desktop/SCAMS/Data_Compiled' '/' 'EEG_Artifact_Mat.mat'])
...
%     end
    ...
    % import the artifact rej arrays from the main analysis where we do
    % have AAR 
    eeg.arf.artIndCleaned = all_arf(sjNum,iSession).arf;

function IEM_Fixed_All_Freqs_Evoked_Perm_Frontal_Chans_Only(sjNum)
...
for iSession=1:2
    ...
    % set a vector of channel locs [frontal midline only]
    frontalMidlineChans = [3,4,11,38,38,53,36,39,46];
    
    % reject artifact trials from eeg and beh
    eegs = eeg.data(frontalMidlineChans,:,:); % just include frontal midline channels
...
for f = 1:nFreqs
    ...
    filterorder = 3;
    type = 'bandpass';
    [z1,p1] = butter(filterorder, [f,f+1]./(EEG.srate/2),type);
    %freqz(z1,p1,[],250)
...
%             
            ...
            for ii = 1:nBins
                for iii = 1:nBlocks
                    ...
                    blockDat_total(bCnt,:,:) = abs(squeeze(mean(fdata_total(permedPosBin==posBins(ii) & blocks==iii,:,tois),1))).^2;
                    ...
                    if (iii == 1 || iii == 2) % cond1 condition
                        ...
                        blockDat_total_cond1(cond1_bCnt,:,:) = abs(squeeze(mean(fdata_total(permedPosBin==posBins(ii) & blocks==iii & perm_Trials == 1,:,tois),1))).^2;
                    ...
                    elseif (iii == 3 || iii == 4) % Exercise condition
                        ...
                        blockDat_total_cond2(cond2_bCnt,:,:) = abs(squeeze(mean(fdata_total(permedPosBin==posBins(ii) & blocks==iii& perm_Trials == 2,:,tois),1))).^2;
                    ...
                    elseif (iii == 5 || iii == 6) % Exercise condition
                        ...
                        blockDat_total_cond3(cond3_bCnt,:,:) = abs(squeeze(mean(fdata_total(permedPosBin==posBins(ii) & blocks==iii& perm_Trials == 3,:,tois),1))).^2;
                    ...
                    elseif (iii == 7 || iii == 8) % Exercise condition
                        ...
                        blockDat_total_cond4(cond4_bCnt,:,:) = abs(squeeze(mean(fdata_total(permedPosBin==posBins(ii) & blocks==iii& perm_Trials == 4,:,tois),1))).^2;
...
if processForIEM==1
    ...
    fName = [root, '/IEM_Results_TT_Within_Fixed_All_Freqs_Evoked_Perm_Frontal_Chans_Only/',sprintf('sj%02d_fixed_IEM.mat',sjNum)];

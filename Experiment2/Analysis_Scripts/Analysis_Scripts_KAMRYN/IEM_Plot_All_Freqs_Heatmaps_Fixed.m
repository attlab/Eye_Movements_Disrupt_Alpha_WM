...
% matlab cluster)
load([slopeDataDir '/' 'IEM_Slopes_All_Freqs.mat'])
...
load([bfTestDir '/' 'BF_Results_IEM_All_Freqs_RvP.mat'])
for iCond=1:size(rSl_total,2)
    for iFreq=1:size(rSl_total,3)
        for iTime=1:size(rSl_total,4)
...
for iPlot=1:4
    ...
    ih = imagesc(squeeze(rSl_Adj(iPlot,1:30,:)),[0,.003]);
    
    set(gca,'xtick',[1:640/5:640,640],...

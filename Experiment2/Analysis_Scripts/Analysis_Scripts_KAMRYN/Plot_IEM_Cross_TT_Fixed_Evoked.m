...
sourceDirReal = [rDir '/' 'IEM_Results_TT_Within_Fixed_Gen_Evoked'];
sourceDirPerm = [rDir '/' 'IEM_Results_TT_Within_Fixed_Gen_Perm_Evoked'];
...
for iSub=1:length(subs)
    ...
    allTF_real_total(iSub,:,:,:)=em.rSl.evoked; %subs x cross x tr x te
    
    clear em
    
    % load data
    load([sourceDirPerm '/' sprintf('sj%02d_fixed_IEM.mat',sjNum)])
    allTF_perm_total(iSub,:,:,:)=em.pSl.evoked; %subs x cross x tr x te
...
% load stats
load([sourceDirStats '/' 'Cross_TT_Slope_BF_TTESTS_Fixed_Evoked.mat'])
...
saveas(h,[destDirPlot '/' 'IEM_Temporal_Gen_Fixed_Model_Evoked.eps'],'epsc');
saveas(h,[destDirPlot '/' 'IEM_Temporal_Gen_Fixed_Model_Evoked.jpg'],'jpeg');

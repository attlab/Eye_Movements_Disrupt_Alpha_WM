...
% set subjects
subjects = [1:2,4:17,20,21:24,27:28];
...
meanTrialRej = squeeze(nanmean(nanmean(trialRejMat,2),1));
semTrialRej = squeeze(nanstd(nanmean(trialRejMat,2),0,1)./sqrt(25));

% get channel rej stats
meanChanRej = nanmean(nanmean(chanRejMat,1),2)/64*100;
semChanRej = nanstd(nanmean(chanRejMat,2),0,1)./sqrt(25);

save([destDir '/' 'Rejection_Summary_Stats.mat'],'meanTrialRej','semTrialRej','meanChanRej','trialRejMat','chanRejMat','subjects')

function IEM_Fixed_All_Freqs_Evoked_Frontal_Chans_Only(sjNum)
...
for f = 1:nFreqs
    ...
    fdata_total = eegs; % SHOULD REALLY BE EVOKED 
        ...
        for ii = 1:nBins
            for iii = 1:nBlocks
                ...
                if (iii == 1 || iii == 2) % cond1 condition
                    ...
                    blockDat_total_cond1(cond1_bCnt,:,:) = abs(squeeze(mean(fdata_total(posBin==posBins(ii) & blocks==iii,:,tois),1))).^2; % abs here unnecessary
...
if processForIEM==1
    ...
    fName = [root, '/IEM_Results_TT_Within_Fixed_All_Freqs_Evoked_Frontal_Chans_Only/',sprintf('sj%02d_fixed_IEM.mat',sjNum)];

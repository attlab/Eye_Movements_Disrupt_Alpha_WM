function IEM_Fixed_Perm_No_AAR(sjNum,iSpatialOrColor)
...
% setup directories
root = '/home/waldrop/Desktop/SCAMS';
eegDir = [root '/' 'EEG_Prepro2_Avg_Baseline_No_AAR'];
...
for iSession=1:2
    ...
    for b=1:length(badChannels)
        for c=1:length(eeg.chanlocsOriginal)
            if strcmp(eeg.chanlocsOriginal(c).labels,badChannels{b})
                cnt=cnt+1;
                badChansIdx(cnt) = c;
            end
        end
    ...
    eegs = eeg.data;
    eegs(badChansIdx,:,:) = []; % rejects channels
...
if processForIEM==1
    ...
    if iSpatialOrColor==1
        fName = [root, '/IEM_Results_TT_Within_Fixed_Perm_No_AAR/',sprintf('sj%02d_fixed_IEM.mat',sjNum)];
    else
        fName = [root, '/IEM_Results_TT_Within_Fixed_Perm_Color/',sprintf('sj%02d_fixed_IEM.mat',sjNum)];
    end

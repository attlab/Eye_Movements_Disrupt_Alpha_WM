...
% matlab cluster)
load([slopeDataDir '/' 'IEM_Slopes_All_Freqs_Fixed_Evoked_Frontal_Chans_Only.mat'])

% average across subs
rSl = squeeze(mean(rSl_evoked,1));


% loop through real slope mat and zero out any p<.05 (frequentist)
if thresholdType==1
    for iCond=1:size(rSl_evoked,2)
        for iFreq=1:size(rSl_evoked,3)
            for iTime=1:size(rSl_evoked,4)
                
                if ttest(rSl_evoked(:,iCond,iFreq,iTime),pSl_evoked(:,iCond,iFreq,iTime))
...
if thresholdType==2
    load([bfTestDir '/' 'BF_Results_IEM_All_Freqs_RvP_Fixed_Evoked.mat'])
    for iCond=1:size(rSl_evoked,2)
        for iFreq=1:size(rSl_evoked,3)
            for iTime=1:size(rSl_evoked,4)
...
%         'box','off')
    ...
    saveas(h,[plotDir '/' 'IEM_All_Freqs_Cond_Evoked_Frontal_Chans_Only' num2str(iPlot) '.eps'],'epsc')
